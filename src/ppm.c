...
inline void set_pixel(const PPMFile *const restrict file, const uint32_t widthPos, const uint32_t heightPos,
                      const Color color) {
    file->pixel_buff[file->width * heightPos + widthPos] = color;
}

void draw_line(const PPMFile *restrict file, uint32_t x0, uint32_t y0, uint32_t const x1, uint32_t const y1,
               Color const color) {
